<template>
  ...
  <!--  <el-radio-group v-model="formData.is_show" placeholder="请选择状态">
        <el-radio 
...
import { apiGoodBrandAdd, apiGoodBrandEdit, apiGoodBrandDetail } from '@/api/good_brand'
...
const popupTitle = computed(() => {
    return mode.value == 'edit' ? '编辑商品品牌' : '新增商品品牌'
})
...
const getDetail = async (row: Record<string, any>) => {
    const data = await apiGoodBrandDetail({
        id: row.id
    })
    setFormData(data)
}
...
const handleSubmit = async () => {
	try{
		...
		const data = { ...formData,  }
		mode.value == 'edit' 
		    ? await apiGoodBrandEdit(data) 
		    : await apiGoodBrandAdd(data)
