      ...
      <el-form class="mb-[-16px]" :model="queryParams" inline>
        ...
        <el-form-item label="出入库类型" prop="type">
          ...
          >
            <el-option label="全部" value=""></el-option>
            <el-option
              v-for="(item, index) in dictData.stock_type"
              :key="index"
              :label="item.name"
              :value="item.value"
            />
...
<script lang="ts" setup name="goodsStockHistoryLists">
import { usePaging } from '@/hooks/usePaging'
import { useDictData } from '@/hooks/useDictOptions'
import {
  apiGoodsStockHistoryLists,
  apiGoodsStockHistoryDelete
} from '@/api/goods_stock_history'
...
// 获取字典数据
const { dictData } = useDictData('stock_type')

// 分页相关
const { pager, getLists, resetParams, resetPage } = usePaging({
  fetchFun: apiGoodsStockHistoryLists,
  params: queryParams
})
...
const handleDelete = async (id: number | any[]) => {
  ...
  await apiGoodsStockHistoryDelete({ id })
  getLists()
}
