...
<!--            <el-option-->
<!--              v-for="(item, index) in dictData.activity_type"-->
<!--              :key="index"-->
<!--              :label="item.name"-->
<!--              :value="item.value"-->
<!--            />-->
    ...
    <el-card class="!border-none" v-loading="pager.loading" shadow="never">
      ...
      <div class="mt-4">
        <el-table :data="pager.lists" @selection-change="handleSelectionChange">
          ...
          <el-table-column label="成团人数" prop="group_num" show-overflow-tooltip />
          <el-table-column label="成团时效(小时)" prop="group_hour" show-overflow-tooltip />
            <el-table-column label="是否自动成团" prop="is_auto">
                <template #default="{ row }">
                    <dict-value
                            :options="dictData.is_auto"
                            :value="row.is_auto"
                            :config="{ label: 'name', value: 'value' }"
                    />
                </template>
...
const { pager, getLists, resetParams, resetPage } = usePaging({
  fetchFun: apiShopActivityLists2,
  params: queryParams
})
