...
<script lang="ts" setup>
	...
	const initMap = () => {
		if (props.formValue.address == '') {
			emitSubmit({ address: '', longitude: '', latitude: '', province: '', city: '', county: '' })
		} else {
		...
		console.log(props.formValue,'rrr')
		if (props.formValue.latitude != '' && props.formValue.longitude != '') {
		...
		slist.addEventListener('click', function (event : any) {
			...
			console.log(state.suggestionList[targetindex], 'a2')
			emitSubmit({
				...
				province: state.suggestionList[targetindex].location.province,
				city: state.suggestionList[targetindex].location.city,
				county: state.suggestionList[targetindex].location.district
			})
	...
	const getSuggestions = () => {
		...
		if (keyword.value) {
			jsonp('https://apis.map.qq.com/ws/place/v1/suggestion', {
				key: 'IYFBZ-U2WWJ-SSMFD-XDFXG-2CNQJ-M7FLX',
